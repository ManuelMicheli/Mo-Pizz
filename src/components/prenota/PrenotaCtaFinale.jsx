...
const PrenotaCtaFinale = () => {
    const { prenota } = siteContent;
    const sectionRef = useRef(null);

    useEffect(() => {
        const ctx = gsap.context(() => {
            gsap.from('.pren-cta-finale-elem', {
                y: 30,
                opacity: 0,
                duration: 0.8,
                stagger: 0.12,
    ...
    return (
        <section ref={sectionRef} className="relative py-24 sm:py-36 px-4 sm:px-8 md:px-12 lg:px-20 bg-flame overflow-hidden">
            ...
            <div className="relative max-w-[1400px] mx-auto text-center flex flex-col items-center gap-8 sm:gap-10">
                <h2 className="pren-cta-finale-elem font-playfair font-bold text-cream text-4xl sm:text-5xl md:text-6xl lg:text-7xl leading-tight">
                    {prenota.ctaFinale.headline}
                </h2>
                <p className="pren-cta-finale-elem font-sans text-cream/80 text-lg sm:text-xl md:text-2xl max-w-2xl leading-relaxed">
                    {prenota.ctaFinale.body}
                </p>
                <div className="pren-cta-finale-elem flex flex-col sm:flex-row gap-4 sm:gap-6 w-full sm:w-auto">
                    <button
                    ...
                    >
                        <Phone size={22} />
                        {prenota.ctaFinale.ctaSecondary}
                    </a>
                </div>
            </div>
        </section>
...
export default PrenotaCtaFinale;
