...
const icons = [Clock, Users, MapPin, RefreshCw];

const PrenotaInfoPratiche = () => {
    const { prenota } = siteContent;
    const infoCards = prenota.infoPratiche.cards.map((c, i) => ({ ...c, icon: icons[i] }));
    const sectionRef = useRef(null);

    useEffect(() => {
        const ctx = gsap.context(() => {
            gsap.from('.pren-info-heading', {
                y: 30,
                opacity: 0,
                duration: 0.8,
                ease: 'power3.out',
    ...
    return (
        <section ref={sectionRef} className="py-24 sm:py-36 px-4 sm:px-8 md:px-12 lg:px-20 bg-charcoal">
            <div className="max-w-[1400px] mx-auto">
                <div className="text-center mb-16 sm:mb-20 pren-info-heading">
                    <span className="font-caveat text-gold text-2xl sm:text-3xl">{prenota.infoPratiche.eyebrow}</span>
                    <h2 className="font-playfair font-bold text-cream text-4xl sm:text-5xl md:text-6xl lg:text-7xl mt-3">
                        {prenota.infoPratiche.headline}
                    </h2>
                </div>

                <div className="pren-info-cards-grid grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5 sm:gap-6">
                    {infoCards.map((card) => (
                        <div
                            key={card.title}
                            className="pren-info-card bg-white rounded-[2rem] p-6 sm:p-8 flex flex-col gap-5"
                        >
...
export default PrenotaInfoPratiche;
