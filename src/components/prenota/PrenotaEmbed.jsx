...
import { ExternalLink, Shield, CalendarCheck } from 'lucide-react';
...
const PrenotaEmbed = () => {
    ...
    useEffect(() => {
        const ctx = gsap.context(() => {
            ...
            gsap.from('.pren-embed-card', {
                ...
                ease: 'power3.out',
                scrollTrigger: {
                    trigger: '.pren-embed-card',
                    start: 'top 85%',
                },
            });

            gsap.from('.pren-embed-trust', {
                y: 20,
                opacity: 0,
                duration: 0.8,
                stagger: 0.1,
    ...
    return (
        <section ref={sectionRef} id="prenota-form" className="w-full py-28 sm:py-40 bg-charcoal relative overflow-hidden">
            ...
            <div className="w-full max-w-[1600px] mx-auto px-6 sm:px-12 md:px-16 lg:px-24 relative z-10">
                {/* Section Header */}
                <div className="text-center mb-14 sm:mb-20 pren-embed-heading">
                    ...
                    <p className="font-sans text-smoke/80 text-base sm:text-lg mt-5 max-w-lg mx-auto leading-relaxed">
                        Compila il form e ricevi la conferma della tua prenotazione in pochi istanti.
                    </p>
                    <div className="w-16 h-[2px] bg-gradient-to-r from-transparent via-flame to-transparent mx-auto mt-6" />
                </div>

                {RESERVE_MODE === 'iframe' ? (
                    <div className="pren-embed-card relative w-full max-w-4xl mx-auto rounded-[1.5rem] sm:rounded-[2.5rem] border border-white/10 bg-white/[0.03] backdrop-blur-sm overflow-hidden shadow-2xl shadow-black/30">
                        {/* Top accent line */}
                    ...
                    <div className="pren-embed-card w-full max-w-3xl mx-auto rounded-[2.5rem] border border-white/10 bg-white/[0.03] backdrop-blur-sm overflow-hidden">
                        {/* Top accent line */}
                        ...
                        <div className="flex flex-col items-center text-center gap-8 py-16 sm:py-24 px-8">
                            ...
                            <div className="w-20 h-20 rounded-2xl bg-flame/10 border border-flame/20 flex items-center justify-center">
                                <CalendarCheck className="text-flame" size={36} />
                            </div>

                            <p className="font-sans text-cream/80 text-lg sm:text-xl max-w-lg leading-relaxed">
                                Prenota il tuo tavolo da Mo Pizz su Plateform. Scegli data, orario e numero di coperti in pochi click!
                            </p>
                            ...
                            <a
                                ...
                                target="_blank"
                                rel="noopener noreferrer"
                                className="magnetic-btn bg-flame hover:bg-ember text-cream font-sans font-bold py-4 px-12 rounded-full text-lg flex items-center gap-3 transition-all duration-300 hover:shadow-[0_0_40px_rgba(232,93,38,0.25)]"
                            >
                ...
                {/* Trust bar */}
                <div className="pren-embed-trust-bar mt-12 sm:mt-16 flex flex-wrap items-center justify-center gap-6 sm:gap-10">
                    <div className="pren-embed-trust flex items-center gap-2.5 text-smoke/40">
                        <Shield size={16} />
                        <span className="font-sans text-xs sm:text-sm">Prenotazione sicura</span>
                    </div>
                    <div className="pren-embed-trust flex items-center gap-2.5 text-smoke/40">
                        <span className="font-mono text-xs sm:text-sm">Conferma immediata</span>
                    </div>
                    <div className="pren-embed-trust flex items-center gap-2.5 text-smoke/40">
                        <span className="font-mono text-xs sm:text-sm">Cancellazione gratuita</span>
                    </div>
                </div>
            </div>
        </section>
